...
if (isset($_GET['user_id']) && isset($_GET['group_id']) && isset($_GET['time']) && isset($_GET['page']) && isset($_GET['limit']))
{
        ...
        && (!!preg_match('/^[0-9]*$/',$page) && $page > 0) && (!!preg_match('/^\d{1,3}$/',$limit)) && $limit > 1)
    {
        ...
        $query = 'SELECT id,user_id,qq_group_id,message,time FROM messages_' . date('Ymd',$time) . ' WHERE time > ' . $time . $query . ' ' . ' ORDER BY id desc ' . $query_limit;
        ...
        foreach ($data as $key => $value)
        {
            ...
            foreach ($cq_code as $item)
            {
                ...
                {
                    case 'image':
                        $result['data']['url'] = str_replace('https://gchat.qpic.cn',CONFIG['image']['proxy'],$result['data']['url']);
